...
import com.example.guru2.fire.WrongRecord
...
class WrongListFragment : Fragment(R.layout.fragment_wrong_list) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 오답 목록 RecyclerView 설정
        // 세로 스크롤, 날짜별 그룹 표시
        val recycler = view.findViewById<RecyclerView>(R.id.recyclerView)
        recycler.layoutManager = LinearLayoutManager(requireContext())

        // Fragment 생성 시 전달받은 타입
        val type = arguments?.getString(ARG_TYPE) ?: "slang"
        val store = FirestoreWrongNote()
        ...
        store.loadWrongs(
            ...
            onResult = { records ->
                ...
                // 중복 제거하면서 가장 마지막 wrongAt만 남기기
                val dedup = linkedMapOf<String, WrongRecord>()
                for (r in records) {
            ...
            onFail = { e ->
                Log.e("WRONG_FIRE", "오답 불러오기 실패: ${e.message}")
            }
        )
    }
}